"""Firestore Client — API agent helpers for status updates, events, and input reading.

Adapted from step-agents pattern for the API agent base framework (Issue 26).
...
def read_step_config(org_id: str, run_id: str, step_id: str) -> dict:
    """Read the step document to get api, skills, role, instruction, etc."""
    db = _get_db()
    doc = db.collection("orgs").document(org_id) \
        .collection("playbook_runs").document(run_id) \
